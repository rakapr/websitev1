...
export async function getStaticPaths() {
  const paths = ["/catagory/2", "/catagory/2"];
...
export async function getStaticProps({ query, params }) {
  ...
  const res = await fetch(
    `https://staging.apricart.pk/v1/catalog/categories/products?category=${id}&page=${perPage}&size=60&sortType=&sortDirection=desc&instant=3`
  );
